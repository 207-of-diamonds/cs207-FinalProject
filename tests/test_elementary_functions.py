...
def test_sin_result_var():
    ...
    obj_wanted = Variable(name='x', value=np.sin(val), 
                          derivative=np.cos(val), second_derivative=0)
...
def test_tan_result_var():
    ...
    obj_wanted = Variable(name='x', value=np.tan(val), derivative=1/(np.cos(val))**2, second_derivative=0)
    assert np.isclose(obj_generate.val, obj_wanted.val)
    assert list(obj_generate.der) == list(obj_wanted.der)
    assert sum(list(map(lambda x: not np.isclose(obj_generate.der[x], obj_wanted.der[x]), obj_generate.der)))==0
...
def test_log_result_var():
    ...
    obj_wanted = Variable(name='x', value=np.log(val), derivative=1/(val), second_derivative=0)
    assert np.isclose(obj_generate.val, obj_wanted.val)
    assert list(obj_generate.der) == list(obj_wanted.der)
    assert sum(list(map(lambda x: not np.isclose(obj_generate.der[x], obj_wanted.der[x]), obj_generate.der)))==0
...
def test_sqrt_result_var():
    ...
    obj_wanted = Variable(name='x', value=np.sqrt(val), derivative=1/(2*np.sqrt(x.val)), second_derivative=0)
    assert np.isclose(obj_generate.val, obj_wanted.val)
    assert list(obj_generate.der) == list(obj_wanted.der)
    assert sum(list(map(lambda x: not np.isclose(obj_generate.der[x], obj_wanted.der[x]), obj_generate.der)))==0
...
def test_arccos_result_var():
    ...
    obj_wanted = Variable('x',np.arccos(val), -1/np.sqrt(1-val**2))
    assert np.isclose(obj_generate.val, obj_wanted.val)
    ## ensure same element
    assert list(obj_generate.der)==list(obj_wanted.der)  
    ## ensure same value
...
def test_arctan_result_var():
    ...
    obj_wanted = Variable('x', np.arctan(val), 1/(val**2+1))
    assert np.isclose(obj_generate.val, obj_wanted.val)
    ## ensure same element
    assert list(obj_generate.der)==list(obj_wanted.der)  
    ## ensure same value
...
def test_exp_result_var():
    ...
    obj_wanted = Variable('x', np.exp(val), np.exp(val))
    assert np.isclose(obj_generate.val, obj_wanted.val)
    ## ensure same element
    assert list(obj_generate.der)==list(obj_wanted.der)  
    ## ensure same value
...
def test_sihn_result_var():
    ...
    obj_wanted = Variable(name='x', value = np.sinh(val), 
                          derivative = np.cosh(val), second_derivative=0)
    assert np.isclose(obj_generate.val, obj_wanted.val)
    ## ensure same element
    assert list(obj_generate.der)==list(obj_wanted.der)  
    ## ensure same value
...
def test_cosh_result_var():
    ...
    obj_wanted = Variable(name='x', value= np.cosh(val), 
                          derivative = np.sinh(val), second_derivative=0)
    assert np.isclose(obj_generate.val, obj_wanted.val)
    ## ensure same element
    assert list(obj_generate.der)==list(obj_wanted.der)  
    ## ensure same value
...
def test_tanh_result_var():
    ...
    obj_wanted = Variable(name='x', value=np.tanh(val), 
                          derivative = (1/np.cosh(val))**2, second_derivative=0)
    assert np.isclose(obj_generate.val, obj_wanted.val)
    ## ensure same element
    assert list(obj_generate.der)==list(obj_wanted.der)  
    ## ensure same value
